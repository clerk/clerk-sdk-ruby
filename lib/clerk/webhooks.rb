  ...
  class Webhooks
    ...
    def create_svix_app(retries: nil, timeout_ms: nil)
      ...
      if Utils.match_status_code(http_response.status, ['200'])
        if Utils.match_content_type(content_type, 'application/json')
          ...
          obj = Crystalline.unmarshal_json(JSON.parse(response_data), Models::Components::SvixURL)
          response = Models::Operations::CreateSvixAppResponse.new(
    ...
    def delete_svix_app(retries: nil, timeout_ms: nil)
      ...
      if Utils.match_status_code(http_response.status, ['204'])
        ...
        return Models::Operations::DeleteSvixAppResponse.new(
          status_code: http_response.status,
          content_type: content_type,
          raw_response: http_response
        )
    ...
    def generate_svix_auth_url(retries: nil, timeout_ms: nil)
      ...
      if Utils.match_status_code(http_response.status, ['200'])
        if Utils.match_content_type(content_type, 'application/json')
          ...
          response_data = http_response.env.response_body
          obj = Crystalline.unmarshal_json(JSON.parse(response_data), Models::Components::SvixURL)
          response = Models::Operations::GenerateSvixAuthURLResponse.new(
            status_code: http_response.status,
            content_type: content_type,
            raw_response: http_response,
            svix_url: obj
