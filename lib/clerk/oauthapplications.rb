...
module Clerk
  ...
  class OauthApplications
    ...
    def upload_logo(oauth_application_id:, body: nil, retries: nil, timeout_ms: nil, http_headers: nil)
      # upload_logo - Upload a logo for the OAuth application
      ...
      request = Models::Operations::UploadOAuthApplicationLogoRequest.new(
        oauth_application_id: oauth_application_id,
        body: body
      )
      url, params = @sdk_configuration.get_server_details
      base_url = Utils.template_url(url, params)
      url = Utils.generate_url(
        Models::Operations::UploadOAuthApplicationLogoRequest,
        base_url,
      ...
      if Utils.match_status_code(http_response.status, ['200'])
        if Utils.match_content_type(content_type, 'application/json')
          ...
          response_data = http_response.env.response_body
          obj = Crystalline.unmarshal_json(JSON.parse(response_data), Models::Components::OAuthApplication)
          response = Models::Operations::UploadOAuthApplicationLogoResponse.new(
            status_code: http_response.status,
            content_type: content_type,
            raw_response: http_response,
            o_auth_application: obj
    ...
    def revoke_token(body:, oauth_application_id:, retries: nil, timeout_ms: nil, http_headers: nil)
      # revoke_token - Revoke an OAuth application token
      ...
      request = Models::Operations::RevokeOAuthApplicationTokenRequest.new(
        oauth_application_id: oauth_application_id,
        body: body
      )
      url, params = @sdk_configuration.get_server_details
      base_url = Utils.template_url(url, params)
      url = Utils.generate_url(
        Models::Operations::RevokeOAuthApplicationTokenRequest,
        base_url,
      ...
      if Utils.match_status_code(http_response.status, ['204'])
        ...
        return Models::Operations::RevokeOAuthApplicationTokenResponse.new(
          status_code: http_response.status,
          content_type: content_type,
          raw_response: http_response
        )
