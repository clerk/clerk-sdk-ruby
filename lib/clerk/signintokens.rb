  ...
  class SignInTokens
    ...
    def create(request: nil, retries: nil, timeout_ms: nil, http_headers: nil)
      ...
      if Utils.match_status_code(http_response.status, ['200'])
        if Utils.match_content_type(content_type, 'application/json')
          ...
          obj = Crystalline.unmarshal_json(JSON.parse(response_data), Models::Components::SignInToken)
          response = Models::Operations::CreateSignInTokenResponse.new(
      ...
      elsif Utils.match_status_code(http_response.status, ['403', '404', '422'])
        if Utils.match_content_type(content_type, 'application/json')
          http_response = @sdk_configuration.hooks.after_success(
            hook_ctx: SDKHooks::AfterSuccessHookContext.new(
              hook_ctx: hook_ctx
    ...
    def revoke(sign_in_token_id:, retries: nil, timeout_ms: nil, http_headers: nil)
      ...
      url = Utils.generate_url(
        Models::Operations::RevokeSignInTokenRequest,
        base_url,
      ...
      if Utils.match_status_code(http_response.status, ['200'])
        if Utils.match_content_type(content_type, 'application/json')
          ...
          response_data = http_response.env.response_body
          obj = Crystalline.unmarshal_json(JSON.parse(response_data), Models::Components::SignInToken)
          response = Models::Operations::RevokeSignInTokenResponse.new(
            status_code: http_response.status,
            content_type: content_type,
            raw_response: http_response,
            sign_in_token: obj
